...
function createWishlistCard(item) {
    ...
    // 링크 추가 (contentId가 있을 경우 동적으로 생성)
    // const detailLink = item.contentId ?
    //     `<a th:href="@{/attractions/detail/{contentId}(contentId=${item.contentId})}">` : '';
    console.log("Item Content ID: ", item.contentId);
    const detailLink = item.contentId ?
        `<a href="/attractions/detail/${item.contentId}" class="destination-card-link">` : '';
    ...
    card.innerHTML = `
        <div class="card-image-wrapper">
            ...
            <button class="heart-button active" data-attraction-id="${item.id}">
